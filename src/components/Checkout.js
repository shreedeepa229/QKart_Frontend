...
const Checkout = () => {
  ...
  useEffect(() => {
    const onLoadHandler = async () => {
      ...
      const adressload = await getAddresses(token)
            ...
            <Box>
              {/* TODO: CRIO_TASK_MODULE_CHECKOUT - Display list of addresses and corresponding "Delete" buttons, if present, of which 1 can be selected */}
                ...
                No addresses found for this account. Please add one to proceed
                    ...
                    {(addresses) && addresses.all.map((add)=>{
                
                return(
                  <Box key={add._id} className={(addresses.selected===add._id) ? "address-item selected" : "address-item not-selected"} onClick={()=>{
                    setAddresses({...addresses,selected:add._id})
                  }}>
                  <Typography>{add.address}</Typography>
                  <Button  startIcon={<DeleteIcon />} onClick={()=>{deleteAddress(token,add._id)}}>
                    Delete
                  </Button>
                  </Box>
                )
